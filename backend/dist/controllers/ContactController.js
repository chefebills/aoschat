...
exports.list = exports.remove = exports.update = exports.show = exports.storeUpload = exports.store = exports.getContact = exports.index = void 0;
...
exports.store = store;
const storeUpload = async (req, res) => {
    const { companyId } = req.user;
    ...
    const promises = contacts.map(async (contact) => {
        const newContact = { name: contact.Nome, number: contact.Telefone.replace(/\D/g, '') };
        try {
...
exports.list = list;
const createNewContact = async (newContact, companyId, schema) => {
    try {
    ...
    logger_1.logger.info(newContact);
    ...
    if (!validNumber)
        throw new AppError_1.default("Não foi possível localizar o número informado no Whatsapp");
...
const createUploadedContact = async (newContact, companyId, schema) => {
    try {
        await schema.validate(newContact);
    }
    catch (err) {
